...
| `get_shares_info()` | `GetSharesInfoResult` | Overview of pool shares: total nominator funds and share supply, the owner's equity, and how much of it the owner can withdraw right now. `ownerShareValue` is gross owner equity (`totalBalance - nominatorsAmount`), including the punishment-reserve capitalization that exists to absorb recovery losses. The withdrawable `liquidOwnerShare` is what remains after additionally deducting validator punishment reserves, pending payouts, and the storage reserve; it is zero while the pool is halted or insolvent. |
